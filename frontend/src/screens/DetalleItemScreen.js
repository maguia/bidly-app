  ...
  const handlePujar = async () => {
    ...
    if (!item.sinLimitesPuja) {
      ...
      if (montoNum > item.rangoMaximo) {
        Alert.alert('Monto muy alto', `El monto máximo es $${item.rangoMaximo?.toLocaleString('es-AR')}`);
        return;
      }
    } else {
        ...
        {!user && (
          <View style={styles.restriccionBox}>
            ...
            <Text style={styles.restriccionTexto}>
              Solo usuarios registrados pueden ver los precios base e historial de pujas
            </Text>
